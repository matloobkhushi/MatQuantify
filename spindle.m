% s
function [ t, maskedImg_crop, bw3, grayImage, bw5, bwS_cropped, bwD_cropped ] = spindle( img, maskedImg, s, k, bwS, bwDNA )
    ...
    % Calculate regionprops for the spindle with the line through the
    % centre
    v = regionprops(bwS,'Area', 'BoundingBox');
    
    % Crop the masked image
    bwS_cropped = imcrop(bwS, box);
    
    % Calculate regionprops for the DNA with the line through the
    % centre
    u = regionprops(bwDNA, 'Area', 'BoundingBox');
    
    bwD_cropped = imcrop(bwDNA, box);
end